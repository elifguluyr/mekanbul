...
function AddComment() {
  ...
  const onSubmit = (evt) => {
    evt.preventDefault(); // LocalStorage'dan token'ı al
    ...
    if(evt.target.elements.text.value && 
      evt.target.elements.rating.value){
        ...
        let newComment = {
          author: "giriş yapan kullanıcı", // İsim alanı
          text: evt.target.elements.text.value, // Yorum metni
          rating: evt.target.elements.rating.value // Yorum puanı
        }      
        VenueDataService.addComment(id, newComment, token).then((response)=>{
          // Yorum ekleme başarılı ise Redux state'i güncelle
          dispatch({type: "ADD_COMMENT_SUCCESS"});
          // Yorum eklendikten modal göster, kullanıcı kapatınca mekan detayına yönlendirilecek
          setShowModal(true);
        }).catch((e) =>{
